...
// _createFormState - хелпер
const _createFormState = (isDisabled = false, message = '') => ({ isDisabled, message })

// Эта функция проводит валидация данных в форме
const createFormState = ({ price, email, confirmationEmail }, hasAgreedTOS, isNewPurchase) => {
  if (!price || Number(price) <= 0) {
    return _createFormState(true, 'Price is not valid')
  }

  // Если это первая покупка, тогда будем выводить поля для емейла и подтверждения емейла и проверять его
  // Если это повторная покупка после деактивации, тогда поля для емейла и подтверждения емейла выводить не будем
  if (isNewPurchase) {
...
const OrderModal = ({ course, onClose, onSubmit, isNewPurchase }) => {
  ...
  // ПРостаавлена ли галочка "Согласен с условиями..."
  const [hasAgreedTOS, setHasAgreedTOS] = useState(false)

  // Для всех курсов становлена цена 15$. В хуке только запрашивается текущий курс $ / ETH. И 15$ пересчитываются в ETH по текущему курсу
          ...
          <div className="sm:flex sm:items-start">
            <div className="mt-3 sm:mt-0 sm:ml-4 sm:text-left">
              ...
              <div className="mt-1 relative rounded-md">
                ...
                {/* Этот инпут активныый только, если стоит галочка вышк - Adjust Price  */}
                <input
              ...
              <div className="text-xs text-gray-700 flex mt-5">
                ...
                <span>
                  I accept Eincode &apos;terms of service&apos; and I agree that my order can be rejected in the case data provided above are not correct
                </span>
              </div>

              {formState.message && <div className="p-4 my-3 text-red-700 bg-yellow-200 rounded-lg text-sm">{formState.message}</div>}
            </div>
          </div>
        </div>
